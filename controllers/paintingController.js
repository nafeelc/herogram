...
async function generatePaintings(req, res) {
  ...
  try {
    ...
    // TODO: Normally we should add this to a queue and do it and then process
...
async function generateAndLinkIdea(titleId, titleText, instructions, prevIdeas, painting) {
  try {
    // Generate new idea
    const {idea, contextUsed} = await openRouterService.generateIdeas(
      titleId,
      titleText,
      instructions,
      prevIdeas
    ...
    if(contextUsed) {
    await pool.execute(
      'UPDATE paintings SET prompt_context = ? WHERE id = ?',
      [contextUsed, painting.id]
    );
    }
