...
async function loadUserData() {
    try {
        ...
        if (error.response) {
            console.error(
                'LUD: Server error response:',error.response.status,error.response.data);
        }
...
function logout() {
    ...
    // Clear all text fields and other data
    clearAllReferencesAndInputs();

    // Show login form
    showLoginForm();
}

// Delete all references and titles
function clearAllReferencesAndInputs() {
                ...
                if (painting.status === 'completed' && painting.image_url) {
                    ...
                    if (loadingThumb) {
                        switch (painting.status) {
                            case 'pending':
                                loadingThumb.innerHTML = '<div class="status">Pending</div>';
                                break;
                            case 'processing':
                                loadingThumb.innerHTML = '<div class="status">Generating</div>';
                                break;
                            case 'failed':
            ...
            if (painting.status === 'completed' && painting.image_url) {
                ...
                switch (painting.status) {
                    case 'pending':
                        ...
                        break;
                    case 'processing':
                        loadingThumb.innerHTML = '</div><div class="status">Generating...</div>';
                        break;
                    case 'failed':
                        loadingThumb.innerHTML = `
                            <div class="error-icon">❌</div>
...
function showPromptDetails(painting, referenceDataMap) {
    ...
    const referenceThumbnails = document.getElementById('reference-thumbnails');
    const fullPrompt = document.getElementById('full-prompt');
    const imageContext = document.getElementById('context-details');
    const promptContext = document.getElementById('prompt_context');
    ...
    imageContext.textContent = painting.context ? JSON.stringify(painting.context) : 'Context not available';
    promptContext.textContent = painting.prompt_context ? JSON.stringify(painting.prompt_context) : 'Prompt context not available';
